...
Mirrors sqlmq.create_queue's server-side rules so we can fail fast with a
clear Python error before the round-trip to SQL Server.
"""
...
# Name must start with a letter or underscore, contain only [A-Za-z0-9_],
# and be at most 60 characters (to leave room for derived constraint and
# index names within SYSNAME's 128-char limit).
